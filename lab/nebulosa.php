		...
		for(var a=0;a<colors.length;a++){
			...
			if(circles[a]==undefined){
				...
				for(var i=0;i<total;i++){
					...
					circles[a].push({rad : Math.random()*this.maxInitRad, dx : Math.sin(Math.random()*100+i+a), dy : Math.cos(Math.random()*100+i+a), x : Math.random()*this.canvas.WIDTH, y : Math.random()*this.canvas.HEIGHT, color : colors[a]});
				}	
			}
...
$(document).ready(function(e) {
	...
	var text = new TextEffects();
	/*
	// Uncomment block for dat.gui
	var gui = new dat.GUI();
	gui.add(text, 'tolerance',0,1);
	gui.add(text, 'minRad',0,100);
	gui.add(text, 'maxRad',0,100);
	gui.add(text, 'maxSpeed',0,10);
	gui.add(text, 'variation',0,5);
	gui.add(text, 'attraction',-0.00001,0.00001);^*/
...
<canvas width="800" height="500" id="bg" style="color:#09F"></canvas>